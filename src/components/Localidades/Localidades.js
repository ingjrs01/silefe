...
const Localidades = () => {
    ...
    const downloadFile = () => {
        fetchAPIData('/silefe.municipio/filter', formatPost(items,false), referer).then(({data}) => { 
            exportToCsv(data,  items.fields.table, "municipios.csv");    
        });
    ...
    const fetchData = async () => {
        const { data, totalPages, totalItems, page } = await fetchAPIData('/silefe.municipio/filter', formatPost(items), referer);
