...
const Edades = () => {
    ...
    const downloadFile = () => {
        fetchAPIData('/silefe.edad/filter', formatPost(items,false), referer).then(({data}) => { 
            exportToCsv(data,  items.fields.table, "edades.csv");    
        });
    }
    ...
    const fetchData = async () => {
        const { data, totalPages, totalItems, page } = await fetchAPIData('/silefe.edad/filter', formatPost(items), referer);
